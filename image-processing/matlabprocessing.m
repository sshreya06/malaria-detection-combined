...
image_folder = "C:\Users\HP\Downloads\malaria-detection-combined-main\healthy_and_infected\test\infected";
image_files = dir(fullfile(image_folder, '*.png'));

for k = 1:length(image_files)    
    im = imread(fullfile(image_folder, image_files(k).name));
    ...
    im_areamask = imcomplement( imbinarize(rgb2gray(im)) );
    ...
    if ~isempty(final_centers)
        plot(final_centers(:,1), final_centers(:,2), 'r+', ...
            'MarkerSize', 12, 'LineWidth', 2);
    end
    
    title(['Detected Cells - ', image_files(k).name]);
    hold off;
    pause(0.5); 
end